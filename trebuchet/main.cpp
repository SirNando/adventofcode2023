...
// If the ascii code of the character I receive is within the range of numbers, it is a number
...
// Given a string and a starting position, checks if there is a number written within.
// Returns the number or -1
int containsNumber(int startingPosition, int maxPosition, const char *string)
{
    // Check how many positions left
    // We can have 3, 4, or 5
    // Check for six, ten, two, one
    const int remainingPositions = maxPosition - startingPosition;
    if (remainingPositions >= 3)
    {
        // Copy chars between positions to check if they are equal
        char candidateNumber[remainingPositions];
        strncpy(candidateNumber, string + startingPosition, 3);
        candidateNumber[3] = 0;

        // Check for numbers
        if (strcmp("one", candidateNumber) == 0)
            return 1;
        if (strcmp("two", candidateNumber) == 0)
            return 2;
        if (strcmp("six", candidateNumber) == 0)
            return 6;
        if (strcmp("ten", candidateNumber) == 0)
            return 10;
    }
    ...
    {
        char candidateNumber[remainingPositions];
        strncpy(candidateNumber, string + startingPosition, 4);
        candidateNumber[4] = 0;

        // Check for numbers
        if (strcmp("four", candidateNumber) == 0)
            return 4;
        if (strcmp("five", candidateNumber) == 0)
            return 5;
        if (strcmp("nine", candidateNumber) == 0)
            return 9;
    }

    // Check for three, seven, eight
    if (remainingPositions >= 5)
    {
        char candidateNumber[remainingPositions];
        strncpy(candidateNumber, string + startingPosition, 5);
        candidateNumber[5] = 0;

        // Check for numbers
        if (strcmp("three", candidateNumber) == 0)
            return 3;
        if (strcmp("seven", candidateNumber) == 0)
            return 7;
        if (strcmp("eight", candidateNumber) == 0)
            return 8;
    }
...
int main(int, char **)
{
    ...
    for (int coordinate = 0; coordinate < 1000; coordinate++)
    {
        ...
        const int stringLength = strlen(coordinates[coordinate]);

        // Check forward
        for (int character = 0; character < stringLength; character++)
        {
            if (isNumber(coordinates[coordinate][character]))
            {
                char stringNumber = coordinates[coordinate][character];
                int actualNumber = stringNumber - '0';
                firstNumber = 10 * actualNumber;
                break;
            }

            const int number = containsNumber(character, stringLength, coordinates[coordinate]);
            if (number >= 0)
            {
                firstNumber = 10 * number;
                break;
            }
        }

        // Check backwards
        for (int character = stringLength - 1; character >= 0; character--)
        {
            if (isNumber(coordinates[coordinate][character]))
            {
                char stringNumber = coordinates[coordinate][character];
        ...
        const int finalNumber = firstNumber + secondNumber;
        globalCount += finalNumber;
    }
    cout << "Final number: " << globalCount << endl;
}
